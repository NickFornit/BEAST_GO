...
Что будет, если Стимул последовал сразу за предыдущим,
не дожидаясь ответа с периодом ожидания? Что делать с объективным Правилом?
Если ответ еще не формировался, то просто ответ будет формироваться на последний стимул,
а если уже сформировался и запущен (оператор проигнорировал), то новый стимул окажется невпопад
и Правило окажется неадекватным.
*/
