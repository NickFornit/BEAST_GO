/* для последнего 4-го уровня дерева понимания
var lev4,_=createPurposeImageID(0,ps.veryActual,ps.targetID,ps.actionID.ID)
образ желаемой цели для Дерева понимания (дерева ментальных автоматизмов)
...
/* образ цели действия
Сначала повторяет PurposeGenetic и включается как 4-й узел дерева понимания, но
потом может произвольно выбираться иным из существующего набора для переактивации дерева понимания
*/
type PurposeImage struct {
	...
	actionID int //выбранный образ действия бота для достижения данной цели (ActionsImage)
}
