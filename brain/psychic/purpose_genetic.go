 ...
 */
	...
	if len(actualRelextActon)>0{
		_,atmz:=CreateNewlastActionsImageID(0,actualRelextActon,nil,0,0)
		pg.actionID = atmz
	}else {
 ...
 */
		...
		//if veryActualSituation { // нужно хоть что-то сделать, ПАНИКА
			ActID:=[]int{21} // паника
			_,atmz:=CreateNewlastActionsImageID(0,ActID,nil,0,0)
			pg.actionID = atmz
		//}
	}
...
func findAnySympleRandActions()(*Automatizm){
	...
	if len(actID)>0 {
		...
		_,trig := CreateNewlastActionsImageID(0,actArrId,nil,0,0)
...
//!? При создании нового автоматизма с фразой вписывать Tnn: тон настроение, которое брать из текущего гомеостаза ?
		...
		_,trig := CreateNewlastActionsImageID(0,nil,v.PhraseID,v.ToneID,v.MoodID)
		purpose.actionID = trig
		atmzm := createAndRunAutomatizmFromPurpose(&purpose)
		return atmzm
	}
