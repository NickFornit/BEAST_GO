...
// новый кадр эпизодической памяти, сохраняющий
	...
	////////////////////////////////////////////
	// в активной ветке нет ментального автоматизма - нужна субъектиная оценка ситуации новизны
	if kind < 2 {// нет условий для проявления произвольности, найти решение, особенно не заморачиваясь (первое приближение).
...
// Если для данного случая есть подходящая доминанта, то - перейти к ее решению
	...
	///////////////////////////////////////////////
	// в активной ветке ЕСТЬ ментальный автоматизм, который нужно проверить перед выполненением
	// штатный мент.автоматизм, привязанный к ветке: currentMentalAutomatizmID
	if kind==2 { //условия для применения произвольности, если старый мент.автоматизм будет заменен на новый
